...
byobu new-session -d -s "$BYOBU_SESSION_NAME" "roslaunch -p $LOCAL_PORT formation_control formation_topside.launch local_simulation:=false && basl -l"

# start every formation control node
for (( i=0; i<${len}; i++ ));
do
    comm="export ROS_MASTER_URI=http://${VEH_IP[$i]}:${VEH_PORT[$i]} && roslaunch formation_control formation_pladypos.launch veh_name:=${VEH_NAME[$i]} veh_id:=$i local_simulation:=false && bash -l"
    #byobu new-window -n "${VEH_NAME[$i]} fc" -t "$BYOBU_SESSION_NAME" "$comm"
    #TODO remotely start formation control 
done
